    ...
    public static partial class RepositoryExtantions
    {
        ...
        private static VMAphorism getAphorism(dynamic model)
        {
            return new VMAphorism
            {
                ...
                Author = model.AuthorId != null ? new VMAuthorAphorism { Id = model.AuthorId, Name = model.AuthorName, PictureId = model.AuthorPictureId } : null,
        ...
        public static VMAphorismCategory[] GetAphorismCategories(this RepoAphorism repo, string curCategoryId=null)
        {
            using (var conn = new SqlConnection(repo.ConnectionString))
            {
                var data = conn.Query<VMAphorismCategory>("SELECT*FROM get_aphorism_categories(@curCat)", new { curCat = curCategoryId }).ToArray();
                return data;
            }
        }
    }
