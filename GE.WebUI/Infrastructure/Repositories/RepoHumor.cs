...
//        }
       ...
       LEFT JOIN AspNetUsers AS anu ON anu.Id=dm.UserId ";
            ...
            {
                var data = conn.Query<VMHumor, SxVMAppUser, VMHumor>(sb.ToString(), (da, anu) => {
                    da.User = anu;
                    return da;
                }, param: param);
                ...
                if (data.Any())
                {
                    foreach (var item in data)
                    {
                        ...
                        item.Videos = conn.Query<SxVMVideo>("dbo.get_first_material_video @mid, @mct", new { mid = item.Id, mct = ModelCoreType.Humor }).ToArray();
                    }
                }
