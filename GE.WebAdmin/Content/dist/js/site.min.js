...
function getGridViewData(i,t,e){$grid=$(i),$pager=$grid.find(".sx-gv__pager");var a=$grid.attr("data-ajax-url"),r={};$inputs=$grid.find(".sx-gv__filter-row input"),$inputs.each(function(){$input=$(this);var i=$input.val();if(""!=i){var t=$input.attr("name");r[t]=i}});var n={filterModel:r,order:e,page:t};$.ajax({url:a,data:n,method:"post",success:function(i){$grid.parent().html(i),$('[data-toggle="tooltip"]').tooltip()}})}!function(i){i.fn.sx_gv=function(){window.innerWidth>0?window.innerWidth:screen.width;this.each(function(){var t=i(this);t.on("click",".sx-gv__pager a",function(){$a=i(this),i("<i></i>").addClass("fa fa-spinner fa-spin").prependTo($a),$grid=$a.closest(".sx-gv");var t=$a.closest("li").hasClass("active");if(!t){var e=$a.data("page");if($order=$grid.find(".sx-gv__sort-arow"),1==$order.length){var a=$order.closest("th").data("field-name"),r=$order.data("sort-direction"),n={FieldName:a,Direction:r};getGridViewData($grid,e,n)}else getGridViewData($grid,e)}}),t.on("click",".sx-gv__delete-btn",function(t){$a=i(this);var e=$a.attr("href"),a=i('input[name="__RequestVerificationToken"]').val();return $grid=$a.closest(".sx-gv"),$parent=$grid.parent(),confirm("Удалить запись?")&&i.ajax({url:e,method:"post",data:{__RequestVerificationToken:a},success:function(i){$parent.html(i)}}),t.preventDefault(),!1}),t.on("keypress",".sx-gv__filter-row input",function(t){if(13==t.which){$input=i(this),$grid=$input.closest(".sx-gv");var e=$grid.find(".sx-gv__pager li.active a").data("page");return getGridViewData($grid,e),t.preventDefault(),!1}}),t.on("click",".sx-gv__clear-btn",function(){$a=i(this),$grid=$a.closest(".sx-gv"),$grid.find(".sx-gv__filter-row input").val(null),getGridViewData($grid,1)}),t.on("click","th",function(){if(!i(this).hasClass("sx-gv_first-column")){$grid=i(this).closest(".sx-gv");var t=$grid.find(".sx-gv__pager li.active a").data("page");$order=i(this).find(".sx-gv__sort-arow");var e=i(this).data("field-name"),a="Asc";if(1==$order.length){var r=$order.data("sort-direction");"Asc"==r&&(a="Desc")}var n={FieldName:e,Direction:a};getGridViewData($grid,t,n)}}),t.on("change",".sx-gv__select-all-chbx",function(){$chbx=i(this),$grid=$chbx.closest(".sx-gv");var t=$chbx.prop("checked");$a=$grid.find(".sx-gv__add-from-chbx-btn"),t?$grid.find('input[type="checkbox"]').prop("checked","checked"):$grid.find('tbody input[type="checkbox"]').removeAttr("checked")})})}}(jQuery);
function getGridLookupData(t,n){$content=$(n),$grid=$content.closest(".sx-gvl"),$btn=$grid.find(".sx-gvl__button"),$.ajax({method:"post",url:t,beforeSend:function(){$btn.find("i").show()},success:function(t){$grid.attr("data-is-loaded","true"),$content.html(t),$content.sx_gv(),$grid.find(".sx-gvl__dropdown").show(),$content.find('[data-toggle="tooltip"]').tooltip(),$btn.find("i").hide()}})}!function(t){t.fn.sx_gvl=function(){window.innerWidth>0?window.innerWidth:screen.width;this.each(function(){var n=t(this),o=n.find(".sx-gvl").attr("data-is-single-mode");o&&n.on("click","tbody tr:not(.sx-gv__filter-row)",function(o){$row=t(this),$input=$row.closest(".sx-gvl").prev('input[type="hidden"]'),$textInput=n.find(".sx-gvl__input"),$dropdown=n.find(".sx-gvl__dropdown");var i=$row.attr("data-row-id"),d=$row.attr("data-row-text");$input.val(i),$textInput.val(d),$dropdown.hide()}),n.on("click",".sx-tv tbody tr",function(o){$row=t(this),$input=$row.closest(".sx-gvl").prev('input[type="hidden"]'),$textInput=n.find(".sx-gvl__input"),$dropdown=n.find(".sx-gvl__dropdown");var i=$row.attr("data-id"),d=$row.find("[data-text-field]").text();$input.val(i),$textInput.val(d),$dropdown.hide()}),n.on("click",".sx-gvl__input",function(){$grid=t(this).closest(".sx-gvl");var n=$grid.attr("data-is-loaded");if("true"==n)return $dropdown=$grid.find(".sx-gvl__dropdown"),$dropdown.show(),!1;$content=$grid.find(".sx-gvl__content");var o=$grid.attr("data-ajax-url");getGridLookupData(o,$content)}),n.on("click",".sx-gvl__button",function(){n.find(".sx-gvl__input").trigger("click")})})}}(jQuery);